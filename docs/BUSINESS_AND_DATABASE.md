...
黄金时间线是“每日黄金复盘板”，不是普通文件列表：以日期为唯一横向坐标，把当天的行情观察、走势图图片和笔记模块内容聚合在同一日期节点，方便按时间回看市场变化、交易依据和复盘结论。每日汇总笔记用于长期检索，时间线用于快速浏览。
...
- 只有在时间线新增关联笔记（包括在时间线内新建笔记）时，才创建或更新标题为 `黄金每日复盘 - yyyy-MM-dd` 的自动汇总笔记；单独修改文字备注或上传图片不会生成新的笔记。
...
- 点击任意日期节点可直接添加三类内容：文字备注（当天观察/消息/判断）、图片备注（走势图或资料图片）和笔记备注（关联已有笔记，或在保存时新建一篇笔记并自动关联）。
