...
这份文档是面向开发者、运维人员和后续 AI 对话的入口，描述功能边界、代码位置、数据关系及运行约定。实现有变化时，先修改本文和 `业务与数据库说明.md`，再修改代码或数据库迁移。

项目的建设初衷、长期方向和新增需求评估原则见 [`产品愿景与长期原则.md`](./产品愿景与长期原则.md)。后续产品建议、业务设计和代码实现都应以该文档为长期约束。

可借鉴的开源项目、许可证提醒和建议演进路线见 [`开源参考项目与演进建议.md`](./开源参考项目与演进建议.md)。

## 一、快速阅读顺序

1. `docs/产品愿景与长期原则.md`：产品初衷、长期方向和新需求判断原则。
2. `docs/开源参考项目与演进建议.md`：开源参考项目、许可证提醒和建议路线。
3. `README.md`：技术栈、环境变量和启动方式。
4. `docs/本地一键启动说明.md`：Windows 本地一键启动/停止。
5. `docs/业务与数据库说明.md`：业务规则、权限和迁移基线。
...
| 黄金时间线 | `/timeline` | 日期轨道居中横线、默认今天前后 10 天；日期轨道滚轮横移、卡片拖动、缩放和边缘增量加载；每天可多次添加文字、图片、已有/新建笔记；笔记卡片点击后在当前页弹窗查看详情；只有新增笔记时才同步黄金每日复盘；同日卡片固定同侧并按内容宽度碰撞避让，最多上3行下3行 |
...
erDiagram
  ...
  gold_daily_timeline ||--o{ timeline_entry : contains
...
- 时间线/文件：`gold_daily_timeline`、`timeline_entry`、`timeline_note_relation`、`file_object`、`business_file_relation`。
...
修改业务流程、API、权限码、表字段、字典或页面入口时，必须同步更新本文、`业务与数据库说明.md`、对应 AGENTS 和 Flyway 新迁移，并在提交信息中说明影响范围。
