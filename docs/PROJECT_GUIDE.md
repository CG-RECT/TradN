...
| 笔记 | `/notes` | Markdown 增删查改、自动汇总笔记、导出 Markdown、自动同步解除、英文分号标签输入、标签筛选和彩色展示 |
| 黄金时间线 | `/timeline` | 日期居中横线、默认今天前后 10 天；拖动/缩放/边缘增量加载；每天可多次添加文字、图片、已有/新建笔记；只有新增笔记时才同步黄金每日复盘，内容按最多五行瀑布流布局 |
